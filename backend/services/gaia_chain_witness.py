# -*- coding: utf-8 -*-
"""
GaiaChain Witness — Misma lógica que blockchain/gaia_chain_real.py para uso dentro del backend.
TRL7 / eIDAS QES | SHA256 + IPFS opcional + POST /api/v1/witness.
"""
...
IPFS_API_URL = os.getenv("IPFS_API_URL", "").rstrip("/")


class GaiaChainReal:
    """Witness: hash SHA256 + opcional IPFS + POST a /api/v1/witness."""

    def __init__(self, witness_url: Optional[str] = None, ipfs_url: Optional[str] = None) -> None:
    ...
    def witness_transaction(self, data: Dict[str, Any], coop_id: int) -> Dict[str, Any]:
        ...
        payload = {"hash": data_hash, "coop_id": coop_id, "ipfs_cid": ipfs_cid}
        if not requests:
            return {"ok": True, "tx_hash": data_hash, "ipfs_cid": ipfs_cid, "witness_response": {}}
        try:
            r = requests.post(f"{self.witness_url}/api/v1/witness", json=payload, timeout=15)
            if r.status_code in (200, 201, 202):
                return {"ok": True, "tx_hash": data_hash, "ipfs_cid": ipfs_cid, "witness_response": r.json() if r.text else {}}
            return {"ok": False, "error": f"witness API {r.status_code}: {r.text[:200]}", "tx_hash": data_hash}
        except Exception as e:
