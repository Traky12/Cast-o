...
# ─────────────────────────────────────────────────────────────────────────────
# RGPD art. 25 — Privacy by design: parcel_ref opt-in
# Ref: PRONTUARIO-LEGAL-MAESTRO-ROBOTICS-2026.md §2.1
# parcel_ref (referencia catastral SIGPAC) es dato potencialmente identificable.
# Por defecto se elimina del estado on-chain.
# Para incluirla: CASTUO_ROBOTICS_LAB_CHAIN_INCLUDE_PARCEL_ID=1
# ─────────────────────────────────────────────────────────────────────────────

_PARCEL_REF_OPT_IN = os.getenv("CASTUO_ROBOTICS_LAB_CHAIN_INCLUDE_PARCEL_ID", "0") == "1"


def redact_parcel_ref_if_needed(state: AgroState) -> AgroState:
    """
    Redacta sigpac_ref del estado si el opt-in no está activado.
    Devuelve estado modificado.

    RGPD art. 25 (Privacy by design): el campo identificable se elimina
    del flujo on-chain por defecto. La redacción ES cumplimiento, no
    una violación — no se añade a gdpr_violations.

    Solo se incluye si el DPO aprobó explícitamente:
        CASTUO_ROBOTICS_LAB_CHAIN_INCLUDE_PARCEL_ID=1
    """
    sigpac_ref = state.get("sigpac_ref", "")
    if sigpac_ref and not _PARCEL_REF_OPT_IN:
        return {**state, "sigpac_ref": ""}
    return state
...
def ethical_guard_node(state: AgroState) -> AgroState:
    ...
    # 3. RGPD art. 25 — Privacy by design: parcel_ref opt-in
    # Redaction = compliance, NOT a violation → no añadir a gdpr_violations
    state = redact_parcel_ref_if_needed(state)

    # 4. AI Act: ¿necesita revisión humana?
