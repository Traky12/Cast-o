...
# MVP: JSON file (survives restarts, no PostgreSQL needed).
# Producción: reemplazar _load_registry / _save_registry con queries PostgreSQL.

_REGISTRY_PATH = Path(os.getenv("QR_REGISTRY_PATH", "/tmp/castuo_qr_registry.json"))
...
def _save_registry(registry: dict[str, dict]) -> None:
    try:
        ...
        pass  # Log but never fail a QR generation because of persistence


_registro_qr: dict[str, dict] = _load_registry()
...
async def generar_qr_consumidor(req: SolicitudQR) -> QRTrazabilidadResponse:
    ...
    # Almacenar para verificaciones posteriores (persiste en JSON entre reinicios)
