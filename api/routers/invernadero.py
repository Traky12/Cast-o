...
def _audit(accion: str, lote_id: str, actor_nif: str, recurso: str, datos: dict) -> None:
    """Registra el evento en la bitácora de auditoría (no-blocking, falla silenciosa)."""
    try:
        from services.audit.audit_logger import AuditAction, get_audit_logger
        get_audit_logger().log(accion, lote_id, actor_nif, recurso, datos)
    except Exception:
        pass
...
async def abrir_lote(req: AperturaLote) -> LoteResponse:
    ...
    _audit("APERTURA_LOTE", lote_id, req.operador_nif, f"lote/{lote_id}", {
        "cultivo": req.cultivo, "variedad": req.variedad, "eco_certificado": req.eco_certificado,
        "hash_apertura": hash_apertura,
    })
    return resp
...
async def registrar_solucion_nutritiva(req: SolucionNutritivaReading) -> InvernaderoResponse:
    ...
    _audit("REGISTRO_SOLUCION", req.lote_id, "sistema", f"solucion/{req.zona}", {
        "ph": req.ph, "ec": req.ec_ms_cm, "estado": estado, "alertas": len(alertas),
    })
    return resp
...
async def registrar_clima(req: ClimaInvernadero) -> InvernaderoResponse:
    ...
    _audit("REGISTRO_CLIMA", req.lote_id, "sistema", f"clima/{req.zona}", {
        "co2_ppm": req.co2_ppm, "temp_aire_c": req.temp_aire_c,
        "humedad_pct": req.humedad_relativa_pct, "estado": estado, "alertas": len(alertas),
    })
    return resp
...
async def registrar_fitosanitario(req: RegistroFitosanitario) -> InvernaderoResponse:
    ...
    _audit("REGISTRO_FITOSANITARIO", req.lote_id, req.operador_nif, f"fitosanitario/{req.lote_id}", {
        "tipo": req.tipo, "producto": req.producto, "eco_compatible": payload.get("eco_compatible"),
    })
    return resp
...
async def registrar_cosecha(req: RegistroCosecha) -> InvernaderoResponse:
    ...
    _audit("REGISTRO_COSECHA", req.lote_id, req.operador_nif, f"cosecha/{req.lote_id}", {
        "kg_cosechados": req.kg_cosechados, "destino": req.destino,
        "hash_cierre": payload["hash_cierre_lote"],
    })
    return resp
