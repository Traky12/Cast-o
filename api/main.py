...
async def get_iot_latest(sensor_id: str):
    ...
    Prioridad: TimescaleDB (si disponible) → fallback in-memory.
    """
    # 1. Intentar TimescaleDB cuando está disponible
    if timescale_db.available:
        db_row = timescale_db.get_device_latest(sensor_id, metric="readings")
        if db_row:
            # Reconstruir estructura de evento compatible con el formato API
            raw_metadata = db_row.get("metadata")
            try:
                if isinstance(raw_metadata, str):
                    readings = json.loads(raw_metadata)
                else:
                    readings = raw_metadata or {}
            except Exception:
                readings = {}
            return {
                "sensor_id": sensor_id,
                "timestamp": str(db_row.get("time", "")),
                "source": readings.pop("source", "timescaledb"),
                "readings": readings,
                "received_at": str(db_row.get("time", "")),
                "_source": "timescaledb",
            }
