...
class CastouSovereignOrchestrator:
    ...
    async def check_service_health(self, name: str, endpoint: str) -> ServiceHealthResult:
        ...
        try:
            if endpoint.startswith("postgresql://"):
                return await self._check_postgres_tcp(name, endpoint, start)
    ...
    async def _check_postgres_tcp(
        ...
        """TCP reachability check for PostgreSQL endpoints (postgresql://host:port)."""
        checked_at = datetime.now(timezone.utc).isoformat()
        ...
        if not host:
            return ServiceHealthResult(
                ...
                error=f"Invalid PostgreSQL endpoint: missing host in '{endpoint}'",
            )
        ...
        try:
            ...
            latency_ms = (asyncio.get_event_loop().time() - start) * 1000
            logger.warning("PostgreSQL TCP check timed out for %s (%s:%s)", name, host, port)
            return ServiceHealthResult(
                service=name,
                status=ServiceStatus.UNAVAILABLE,
                latency_ms=round(latency_ms, 2),
                endpoint=endpoint,
                checked_at=checked_at,
                error=f"TCP connect timeout after {connect_timeout}s",
            )
        except OSError as exc:
            latency_ms = (asyncio.get_event_loop().time() - start) * 1000
            logger.warning("PostgreSQL TCP check failed for %s: %s", name, exc)
            return ServiceHealthResult(
                service=name,
                status=ServiceStatus.UNAVAILABLE,
                latency_ms=round(latency_ms, 2),
    ...
    async def route_task(self, task: OrchestratorTask) -> Dict[str, Any]:
        ...
        routing_map = {
            ...
            "thingsdata_ingest": self._route_thingsdata,
        }
    ...
    async def _route_thingsdata(self, task: OrchestratorTask) -> Dict[str, Any]:
        """Procesa telemetría de sensores IoT (ESP32/Raspberry Pi) y la envía a FastAPI."""
        device_id = task.payload.get("device_id", "unknown_device")
        telemetry = task.payload.get("telemetry", task.payload.get("data", {}))
        fastapi_base = "http://fastapi:8000"

        try:
            response = await self.http_client.post(
                f"{fastapi_base}/api/v1/thingsdata/ingest",
                json={
                    "device_id": device_id,
                    "timestamp": task.created_at,
                    "data": telemetry,
                    "metadata": {
                        "source": task.payload.get("source", "LoRaWAN"),
            ...
            return {
                ...
                "platform": "ThingsData",
                "device_id": device_id,
                "bytes_processed": len(str(telemetry)),
                "compliance": "RGPD + AI Act",
            }
        except Exception as exc:
            logger.error("ThingsData ingestion failed for %s: %s", device_id, exc)
            return {
                "task_id": task.task_id,
                "status": "error",
                "error": str(exc),
