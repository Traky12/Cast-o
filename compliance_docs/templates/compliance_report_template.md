...
---
## **📌 1. CONTEXTO**
Este informe documenta el cumplimiento normativo de la **generación de media educativa** con ID **{{ media_id }}**, realizada el {{ generated_at }} en el entorno **{{ system.environment }}** de CASTÚO-SYSTEM™.

**Detalles Técnicos**:
- **Modelo de IA**: Stable Diffusion EU (EUPL-1.2).
- **Hosting**: OVH Cloud (Francia).
- **Almacenamiento**: MinIO (OVH) con cifrado AES-256.
- **Registro**: GaiaChain (TX: [{{ evidence.gaiachain_tx }}](https://explorer.gaiachain.es/tx/{{ evidence.gaiachain_tx }})).
...
| **Art. 30**        | Registro de la actividad en GaiaChain.                                              | TX: {{ evidence.gaiachain_tx }}.                                             |
...
| **Tipo**        | **Detalle** |
|-----------------|-------------|
| GaiaChain       | TX: [{{ evidence.gaiachain_tx }}](https://explorer.gaiachain.es/tx/{{ evidence.gaiachain_tx }}). |
| Wazuh           | Consulta: {{ evidence.wazuh_query }}. |
| Almacenamiento | Bucket: {{ evidence.storage.bucket }}, Key: {{ evidence.storage.object_key }}. |
