...
python -c "from fastapi.testclient import TestClient; from backend.integrations.robotics.lab_stub_app import app; c=TestClient(app); r=c.post('/api/robotics/lab/neuromorphic/hydroponics/infer',headers={'Authorization':'Bearer test-bearer'},json={'humedad':65,'ph':5.8,'ec':1.2,'luz_umol':1200}); print('infer', r.status_code)"
