...
def _get_http_client() -> httpx.Client:
    ...
    # En tests, httpx.Client se parchea como mock/context manager.
    # No cacheamos ese objeto para mantener determinismo entre casos.
    if type(httpx.Client).__module__.startswith("unittest.mock"):
        return httpx.Client(timeout=TIMEOUT).__enter__()
...
def check_api_health() -> bool:
    ...
    try:
        ...
        r = _get_http_client().get(f"{API_URL}/health", headers=headers)
...
def check_telemetry_ingest_lookup() -> bool:
    ...
    while time.time() < deadline:
        try:
            r = _get_http_client().get(url, headers=headers)
