...
# ─────────────────────────────────────────────────────────────────────────────
# Tests ingestión ambiental y control de actuadores
# ─────────────────────────────────────────────────────────────────────────────

class TestSensoresYActuadores:
    def test_datos_sensores_en_rango(self):
        resp = client.post("/api/v1/invernadero/datos-sensores", json={
            "temperatura": 25.0,
            "humedad": 60.0,
            "co2": 950,
            "luz": 500,
    ...
    def test_datos_sensores_fuera_rango_rechazado(self):
        resp = client.post("/api/v1/invernadero/datos-sensores", json={
            "temperatura": 40.0,
            "humedad": 60.0,
            "co2": 950,
        ...
        assert "Temperatura fuera de rango" in resp.text

    def test_control_actuador_estado_valido(self):
        resp = client.post(
            "/api/v1/invernadero/actuadores/ventilacion/encender",
            json={"reason": "temp_alta"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["tipo"] == "ventilacion"
        assert data["estado"] == "encender"

    def test_control_actuador_tipo_invalido(self):
        resp = client.post(
            "/api/v1/invernadero/actuadores/laser/encender",
            json={"reason": "test"},
        )
        assert resp.status_code == 400
        assert "Actuador no soportado" in resp.text
